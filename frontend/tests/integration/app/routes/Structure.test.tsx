...
vi.mock("echarts-for-react", () => ({
  default: ({ option }: { option: { series: Array<{ data: unknown }> } }) => (
    <div data-testid="structure-chart">
...
import Structure from "../../../../src/app/routes/Structure";
...
describe("Structure", () => {
  ...
  it("shows an error banner when the library query fails", () => {
    ...
    expect(
      screen.getByText("Failed to load data for structure analysis."),
    ).toBeInTheDocument();
  });
  ...
  it("renders the blueprint view and lets the user focus a schema branch", () => {
    ...
    expect(screen.getByTestId("structure-chart")).toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "Metadata" }));

    expect(screen.getByTestId("structure-chart").textContent).toContain(
      "Metadata",
    );
  });
