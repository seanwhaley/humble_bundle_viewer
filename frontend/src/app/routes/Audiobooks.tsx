...
export default function Audiobooks() {
  ...
  if (isLoading) {
    return <RouteLoadingState label="Loading audiobooks…" />;
  }
    ...
    <div className={PAGE_STACK_TIGHT_CLASS}>
      <Card surface="panel">
        <CardHeader className="pb-4">
          <PaneHeader
            titleAs="h2"
            title="Browse the audiobook library before you download or sync"
            description="Review titles, narrators, formats, and bundle context first. Bulk browser downloads and advanced local sync stay close by, but only expand when you are ready to act on the current selection."
            eyebrow={<Badge variant="info">Listening workflow</Badge>}
          />
        </CardHeader>
          ...
          <div className={GRID_THREE_COLUMN_CLASS}>
            <div className={INSET_PANEL_COMPACT_CLASS}>
              ...
              <p className={INSET_PANEL_BODY_TEXT_CLASS}>
                {formatNumber(audiobooks.length)} audiobook title{audiobooks.length === 1 ? "" : "s"} match the current filters.
              </p>
            </div>
            <div className={INSET_PANEL_COMPACT_CLASS}>
              <p className={SECTION_EYEBROW_CLASS}>
