      ...
      cell: ({ row }) => (
        <div className="flex gap-2">
          <Button variant="outline" size="sm" className="h-7 px-2" asChild>
            <Link
              to={`/library/other-downloads?search=${encodeURIComponent(row.original.originalSearch)}`}>
    ...
    <div className="flex flex-col space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            {allDownloads.length > 0 ?
              <Button variant="outline" size="sm" asChild>
                <Link
                  to={`/library/other-downloads?search=${encodeURIComponent(product.product_name || product.machine_name || "")}`}>
                  Open downloads
                </Link>
              </Button>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
          ...
          <div>
            ...
            <div className="text-sm font-medium">
              {buildPlatformSummary(allDownloads)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div>
        <div className="mb-3">
          ...
          <p className="text-sm text-muted-foreground">
            Review the contained titles, formats, and quick follow-up actions
            for this purchase.
          </p>
        </div>
        <DataTable
